...
void util_array_dispose(void **ptr, unsigned int n, void (*quit)(void *)){
  int i;
  if(quit != NULL)for(i = 0; i < n; i++) quit(ptr[i]);
  free(ptr);
}
...
void **util_matrix_alloc(unsigned int n, unsigned int m, unsigned int size){
  ...
  for(i = 0; i < n; i++)ptr[i] = util_calloc(m, size);
  return ptr;
}
/**matrix de-allocation utility function*/
void util_matrix_dispose(void ***ptr, unsigned int n, unsigned int m, void (*quit)(void *)){
  int i, j;
  for(i = 0; i < n; i++) {
    if(quit != NULL)for (j = 0; j < m; j++) quit(ptr[i][j]);
    free(ptr[i]);
  }
  free(ptr);
}
